          ...
          <div className="hidden md:flex space-x-8">
            {navItems.map(item => (
              <button
                ...
                className={`nav-link capitalize text-sm font-medium transition-colors duration-300 ${
                  activeSection === item 
                    ? 'text-primary active' 
          ...
          <div className="md:hidden pb-4 space-y-1 fade-in-up">
            {navItems.map(item => (
              <button
                key={item}
                onClick={() => handleNavigate(item)}
                className={`block w-full text-left capitalize py-3 px-4 rounded-lg transition-all duration-300 ${
                  activeSection === item 
